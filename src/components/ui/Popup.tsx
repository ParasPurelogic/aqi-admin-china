...
const Popup = (props: PopupProps) => {
  ...
  return ReactDOM.createPortal(
    <div
      className={cn(
        "popup fixed top-0 left-0 w-full min-h-full max-h-full z-[999999] flex flex-col p-body-padding",
        props.className
      )}
    >
      {/* Wrapper Area */}
      ...
      >
        ...
        <div
          className={cn(
            "content w-full h-[20%] grow bg-white rounded-[1.5rem] sm:rounded-[1.8rem] border-white shadow-[4px_7px_37px_0px_rgba(69,69,69,0.06)] overflow-y-auto  max-md:[--scrollbarWidth:0]",
            "border-[2rem] md:border-[3rem] md:border-r-[1.5rem]",
            "md:pr-[1.5rem]",
...
export function CloseButton({
  ...
  return (
    <div
      className={cn(
        "close-button relative cursor-pointer transition hover:bg-zinc-200 bg-white aspect-square ml-auto w-[4.5rem] h-[4.5rem] rounded-full flex items-center justify-center",
        className
      )}
      onClick={() => onClose?.()}
    >
      <svg
        className="absolute aspect-square w-[40%] top-2/4 left-2/4 -translate-x-2/4 -translate-y-2/4 text-title"
        viewBox="0 0 20 20"
        fill="none"
...
export function StickyHeader({
  ...
  return (
    <div
      className={cn(
        "bg-white sticky top-0 z-[5] text-title flex items-center justify-between gap-[1.5rem] border-b pb-[1.5rem] sm:pb-[2.5rem] mb-[1.5rem] sm:mb-[3.5rem]",
        className
      )}
    >
      {children}
...
Popup.StickyHeader = StickyHeader;
